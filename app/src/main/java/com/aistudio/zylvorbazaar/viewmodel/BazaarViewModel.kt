package com.aistudio.zylvorbazaar.viewmodel
...
import com.aistudio.zylvorbazaar.data.*
...
class BazaarViewModel(application: Application) : AndroidViewModel(application) {
    ...
    init {
        ...
        seedDefaultTestAccounts()
    }

    private fun seedDefaultTestAccounts() {
        viewModelScope.launch(Dispatchers.IO) {
            try {
                if (repository.getUser("user@bazaar.com") == null) {
                    val user = User(
                        ...
                        role = "User",
                        phone = "9876543210",
                        savedAddress = "Connaught Place, New Delhi"
                    )
                    repository.insertUser(user)
                    try {
                        FirebaseFirestore.getInstance().collection("users")
                            .document("user@bazaar.com")
                            .set(user.toMap())
                    } catch (_: Exception) {}
                }

                if (repository.getUser("seller@bazaar.com") == null) {
                    val seller = User(
                        ...
                        role = "Seller",
                        phone = "9876543211",
                        shopName = "Zylvor Demo Mart",
                        shopAddress = "Sector 18, Noida, Uttar Pradesh",
                        shopAddressLat = 28.5708,
                        shopAddressLng = 77.3271,
                        isSellerVerified = true,
                        sellerMobile = "9876543211",
                        sellerAadhaar = "123456789012",
                        sellerBankAccount = "123456789012"
                    )
                    repository.insertUser(seller)
                    try {
                        FirebaseFirestore.getInstance().collection("users")
                            .document("seller@bazaar.com")
                            .set(seller.toMap())
                    } catch (_: Exception) {}
                }

                if (repository.getUser("delivery@bazaar.com") == null) {
                    val delivery = User(
                        email = "delivery@bazaar.com",
                        name = "Demo Delivery Partner",
                        password = "Password@123",
                        role = "DeliveryPartner",
                        phone = "9876543212",
                        deliveryMobile = "9876543212",
                        deliveryAadhaar = "123456789013",
                        deliveryAddress = "Indirapuram, Ghaziabad",
                        deliveryAddressLat = 28.6369,
                        deliveryAddressLng = 77.3694,
                        deliveryBankAccount = "987654321098",
                        deliveryEmergencyContact = "9876543219",
                        deliveryVehicleType = "Bike",
                        deliveryVehicleNumber = "DL 01 AB 1234",
                        isDeliveryPartnerVerified = true
                    )
                    repository.insertUser(delivery)
                    try {
                        FirebaseFirestore.getInstance().collection("users")
                            .document("delivery@bazaar.com")
                            .set(delivery.toMap())
                    } catch (_: Exception) {}
                }
            } catch (e: Exception) {
    ...
    val languages: StateFlow<List<String>> = appConfig
        .map { it.languages }
        .stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), listOf("English", "EspaÃ±ol", "FranÃ§ais", "Deutsch", "Hindi", "Bengali"))
    ...
    fun uploadVideoToCloudinary(
        ...
        viewModelScope.launch {
            try {
                ...
                // Step 1: Read file bytes on IO thread
                val (mimeType, extension, safeSeller, bytes) = withContext(Dispatchers.IO) {
                    val resolver = app.contentResolver
                    val mime = resolver.getType(uri) ?: "video/mp4"
                    val ext = android.webkit.MimeTypeMap.getSingleton().getExtensionFromMimeType(mime) ?: "mp4"
                    val safe = sellerEmail.ifBlank { "unknown" }.replace(Regex("[^A-Za-z0-9_-]"), "_")
                    val b = resolver.openInputStream(uri)?.use { it.readBytes() }
                ...
                @Suppress("UNCHECKED_CAST")
                val mimeStr = mimeType as String
                val extStr = extension as String
                val safeStr = safeSeller as String
                val bytesArr = bytes as ByteArray
                ...
                // Helper: Firebase Storage upload runs on Main so awaitTask() callbacks fire correctly
                suspend fun uploadToFirebase(): String {
                    val storagePath = "seller_videos/$safeStr/video_${System.currentTimeMillis()}.$extStr"
                    val storageRef = com.google.firebase.storage.FirebaseStorage.getInstance().reference.child(storagePath)
                    storageRef.putBytes(bytesArr).awaitTask()
                ...
                if (!isConfigured(cloudName) || !isConfigured(uploadPreset)) {
                    // No Cloudinary config â€” use Firebase directly
                    downloadUrl = uploadToFirebase()
                } else {
                    // Step 2: Try Cloudinary upload on IO thread
                    val cloudinaryResult = withContext(Dispatchers.IO) {
                        try {
                            val publicId = "${safeStr}_video_${System.currentTimeMillis()}"
                            val fileBody = bytesArr.toRequestBody(mimeStr.toMediaTypeOrNull())
                            val multipart = okhttp3.MultipartBody.Builder()
                                .setType(okhttp3.MultipartBody.FORM)
                                .addFormDataPart("file", "video_${System.currentTimeMillis()}.$extStr", fileBody)
                                .addFormDataPart("upload_preset", uploadPreset)
                                .addFormDataPart("public_id", publicId)
                                .apply {
                                    if (isConfigured(folder)) addFormDataPart("folder", folder)
                            ...
                            null // Signal Cloudinary failed â€” will fallback to Firebase
                        }
                    }

                    downloadUrl = cloudinaryResult ?: uploadToFirebase()
                }
